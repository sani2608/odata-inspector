...
function handleRequestStarted(data: {
    ...
    const pendingData: Partial<CapturedRequest> = {
        ...
        duration: undefined,
        _pending: true,
        // For batch requests in standalone mode, add indicator
...
/**
 * Handle metadata content received from background worker (standalone mode).
 * Parses the XML, triggers the onMetadata callback, and updates the request's response.
 */
function handleMetadataContent(data: { url: string; content: string }): void {
    try {
        const parsedMetadata = parseMetadataXml(data.content);

        if (parsedMetadata) {
            const serviceUrl = extractServiceUrl(data.url);
            parsedMetadata.serviceUrl = serviceUrl;
            parsedMetadata.rawXml = data.content;

            capturedMetadata = parsedMetadata;

            console.debug('[NetworkCapture] Standalone: Parsed metadata:', {
                serviceUrl,
                entityTypes: parsedMetadata.entityTypes.length,
                entitySets: parsedMetadata.entitySets.length
            });
            ...
            // Update the metadata request's response with the fetched content
            const metadataRequestId = metadataRequestIds.get(data.url);
            if (metadataRequestId) {
                callbacks?.onPendingUpdate?.(metadataRequestId, {
                    response: {
                        statusCode: '200',
                        statusText: 'OK',
                        headers: {},
                ...
                metadataRequestIds.delete(data.url);
            }
        }
    } catch (error) {
        console.debug('[NetworkCapture] Standalone: Could not parse metadata:', (error as Error).message);
    }
}

// Store for debugger-captured requests (for standalone mode with full data)
const debuggerRequestsMap = new Map<string, {
    requestId: string;
    url: string;
    method: string;
...
/**
 * Handle debugger request started (standalone mode with full capture)
 */
function handleDebuggerRequestStarted(data: {
    requestId: string;
    url: string;
    method: string;
    headers: Record<string, string>;
    ...
    // Store for later when we get response
    debuggerRequestsMap.set(data.requestId, {
        ...data,
    });
    ...
    if (data.postData) {
        try {
            ...
            requestBody = data.postData;
        }
    }

    const parsedUrl = parseODataUrl(data.url);
    const isBatch = data.requestType === 'batch';
    const isMetadata = data.requestType === 'metadata';

    const pendingData: CapturedRequest = {
        id: String(uniqueId),
        _uniqueId: uniqueId,
        url: data.url,
        path: new URL(data.url).pathname,
        name: isBatch ? '$batch' : (isMetadata ? '$metadata' : data.entitySet),
        type: data.requestType as 'batch' | 'metadata' | 'entity',
        isMetadata,
        _isBatchRequest: isBatch,
        request: {
            method: data.method as HttpMethod,
    ...
    callbacks?.onRequest(pendingData);
}

/**
 * Handle debugger response received (headers available)
 */
function handleDebuggerResponseReceived(data: {
    requestId: string;
    statusCode: number;
    statusText: string;
    headers: Record<string, string>;
    ...
    const storedRequest = debuggerRequestsMap.get(data.requestId);
    if (storedRequest) {
        storedRequest.responseHeaders = data.headers;
        storedRequest.statusCode = data.statusCode;
        storedRequest.statusText = data.statusText;
    }

    const pending = pendingRequestsMap.get(data.requestId);
    if (pending?.id) {
        callbacks?.onPendingUpdate?.(pending.id, {
            response: {
                statusCode: String(data.statusCode),
...
/**
 * Handle debugger request complete (body available)
 */
function handleDebuggerRequestComplete(data: {
    requestId: string;
    duration: number;
    ...
    if (!pending?.id) {
        debuggerRequestsMap.delete(data.requestId);
        return;
    }

    // Parse response body
    let responseData: unknown = data.responseBody;
    if (data.responseBody) {
        try {
            const parsed = JSON.parse(data.responseBody);
            // Extract OData payload
            responseData = parsed.d || parsed;
            // Clean up metadata
            responseData = deleteKeyDeep(responseData, '__metadata');
            responseData = deleteKeyDeep(responseData, '__deferred');
        } catch {
    ...
    // Update the request with final data
    callbacks?.onPendingUpdate?.(pending.id, {
        response: {
            statusCode: String(storedRequest?.statusCode || 200),
            statusText: storedRequest?.statusText || 'OK',
    ...
    if (pending.isMetadata && data.responseBody) {
        const parsedMetadata = parseMetadataXml(data.responseBody);
        if (parsedMetadata) {
            const serviceUrl = extractServiceUrl(storedRequest?.url || pending.url || '');
            parsedMetadata.serviceUrl = serviceUrl;
            parsedMetadata.rawXml = data.responseBody;
            capturedMetadata = parsedMetadata;

            callbacks?.onMetadata?.(parsedMetadata);

            // Update with metadata summary
            callbacks?.onPendingUpdate?.(pending.id, {
                metadataSummary: {
                    entityTypes: parsedMetadata.entityTypes.length,
                    entitySets: parsedMetadata.entitySets.length,
            ...
            console.debug('[NetworkCapture] Standalone: Parsed metadata via debugger');
        }
    }
...
/**
 * Handle debugger request failed
 */
function handleDebuggerRequestFailed(data: {
    requestId: string;
    duration: number;
    errorText: string;
}): void {
...
/**
 * Initialize standalone capture for non-DevTools context.
 *
 * ★ Insight ─────────────────────────────────────
 * - Standalone mode uses Chrome debugger API for FULL request/response data
 * - Provides complete parity with DevTools (headers, bodies, timing)
 * - Shows a "debugging" banner but enables full functionality
 * - Users get all features: response bodies, batch parsing, metadata
 * ─────────────────────────────────────────────────
 *
 * @param tabId - The tab ID to monitor (from URL parameter)
 * @param cbs - Callbacks for request events
 */
export function initStandaloneCapture(tabId: number, cbs: NetworkCaptureCallbacks): void {
    callbacks = cbs;

    try {
        backgroundPort = chrome.runtime.connect({ name: 'odata-inspector' }) as BackgroundPort;

        // Send the provided tab ID with standalone flag to enable debugger
        backgroundPort.postMessage({
            type: 'init',
            tabId: tabId,
            standalone: true
        ...
        backgroundPort.onMessage.addListener((msg: unknown) => {
            ...
            switch (message.type) {
                // Debugger messages (full data capture)
                case 'debugger-attached':
                    console.debug('[NetworkCapture] Debugger attached - full capture enabled');
                    break;

                case 'debugger-attach-failed':
                    console.error('[NetworkCapture] Debugger attach failed - falling back to basic capture');
                    break;

                case 'debugger-detached':
                    console.debug('[NetworkCapture] Debugger was detached by user');
                    callbacks?.onClear?.();
                    break;

                case 'debugger-request-started':
                    handleDebuggerRequestStarted(message.data as {
                        requestId: string;
                        url: string;
                        method: string;
                        headers: Record<string, string>;
                ...
                case 'debugger-response-received':
                    handleDebuggerResponseReceived(message.data as {
                        requestId: string;
                        statusCode: number;
                        statusText: string;
                        headers: Record<string, string>;
                ...
                case 'debugger-request-complete':
                    handleDebuggerRequestComplete(message.data as {
                        requestId: string;
                        duration: number;
                        responseBody: string | null;
                    });
                ...
                case 'debugger-request-failed':
                    handleDebuggerRequestFailed(message.data as {
                        requestId: string;
                        duration: number;
                        errorText: string;
                    });
                ...
                // Legacy webRequest messages (fallback)
                case 'request-started':
                    handleRequestStarted(message.data as {
                        requestId: string;
                        url: string;
                        method: string;
                        entitySet: string;
                ...
                case 'metadata-content':
                    handleMetadataContent(message.data as { url: string; content: string });
                    break;
            }
        });

        backgroundPort.onDisconnect.addListener(() => {
            console.debug('[NetworkCapture] Standalone background disconnected, reconnecting...');
            setTimeout(() => initStandaloneCapture(tabId, cbs), 1000);
        });

        console.debug(`[NetworkCapture] Standalone capture initialized for tab ${tabId}`);
    } catch (e) {
        console.error('[NetworkCapture] Could not connect to background:', (e as Error).message);
    }
}
